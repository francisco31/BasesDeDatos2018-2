...
---- Especialidad que cuesta más y los datos de los médicos que la pueden realizar.
...
-- Ventas por tipo de medicamento.
...
-- Número de consultas por turno y sucursal
select *
from (
...
-- Clientes que obtuvierón alguna consultas gratis.
...
-- Mes en el que se atienden más consultas, tomando como fuente todas las consultas realizadas hasta la fecha.
select mes , maximo
from (
...
-- Consultas que generarón más de una receta, y el nombre del médico que atendió tal consulta. 
...
-- Número de consultas que hace cada médico.
...
-- Número de consultas generales atendidas cada mes.
...
-- Médicos que no cuenten con especialidades pero son responsables de alguna sucursal.
...
-- Número de médicos por cada especialidad.
...
-- Datos de los clientes que ya cuentan con su monedero electrónico y tienen saldo de más de $500.
...
-- La especialidad que más es solicitada y el nombre de los médicos que la realizan.
